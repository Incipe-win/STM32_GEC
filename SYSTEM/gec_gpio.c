...
static GPIOMode_TypeDef get_stm32gpiomode(enum GPIO_MODE gpio_mode) {
  ...
  if ((gpio_mode & GPIO_MODE_IN_FLOATING) ||
    ...
    mode = GPIO_Mode_AN;
  } else if ((gpio_mode & GPIO_MODE_OUT_OD) || (gpio_mode & GPIO_MODE_OUT_PP)) {
    mode = GPIO_Mode_OUT;
  }
  ...
  switch (gpio_mode)
  {
          ...
          case GPIO_MODE_IN_PULLDOWN:
                  ...
                  break;
                                                           ...
                                                           //GPIO口配置成EXTI时，in
  floating case GPIO_MODE_EXTI_Both: case GPIO_MODE_EXTI_Rising: case
  GPIO_MODE_EXTI_Falling:
                  //mode = GPIO_Mode_IN_FLOATING;
                  mode = GPIO_Mode_IPU;
                  break;
...
*/
void Gec_GPIO_Init(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin,
                   enum GPIO_MODE gpio_mode) {
  //	int ret = 0;
...
#if STM32F1
  //
  if ((GPIOx == GPIOA && (GPIO_Pin == GPIO_Pin_13 || GPIO_Pin == GPIO_Pin_14 ||
                          GPIO_Pin == GPIO_Pin_15)) ||
      ((GPIOx == GPIOB &&
        (GPIO_Pin == GPIO_Pin_3 || GPIO_Pin == GPIO_Pin_4)))) {
...
// 1.2 SYSCFG控制器
    ...
    SYSCFG_EXTILineConfig(get_gpio_exti_port(GPIOx),
                          get_gpio_exti_pin(GPIO_Pin)); // PA0 -> EXTI0
#elif STM32F1
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_AFIO, ENABLE);
...
void Gec_GPIO_Set_Value(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin,
                        uint16_t value) {
